  ...
  return (
    <div>
      <div className="flex items-center gap-3 px-4 py-3.5 border-b border-hairline">
        ...
        <div className="font-display italic text-[17px]">Followers</div>
      </div>
      {(!follows || follows.length === 0) && (
        <div className="px-4 py-10 text-center text-inksoft text-sm">No followers yet.</div>
      )}
      {(follows || []).map((f) => (
        <Link key={f.follower_id} href={`/profile/${f.profiles?.username}`} className="flex items-center gap-3 px-4 py-3 border-b border-hairline">
          <img
